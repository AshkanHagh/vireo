...
export const searchByUsernameInCache = async (
  username: string,
...
export const updateUserCache = async (
  user: TInferUpdateUser,
): Promise<void> => {
  await addToHashCache(`user:${user.id}`, user, 604800);
  const updatedUser: TInferSelectUserNoPass = await getAllFromHashCache(
    `user:${user.id}`,
  );
  ...
  const updateCacheForPattern = async (pattern: string): Promise<void> => {
    ...
    do {
      ...
      await Promise.all(
        keys.map(async (key) => {
          const followers = await getAllFromHashCache<TCacheIndex>(key);
  ...
  await Promise.all([
    updateCacheForPattern("followers:*"),
    updateCacheForPattern("followings:*"),
  ]);
};
