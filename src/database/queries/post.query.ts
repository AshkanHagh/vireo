...
): Promise<TInferSelectPost> => {
  const createdPost = await db
    .insert(PostTable)
    .values({ userId: currentUserId, text, image: image || undefined })
    .returning();
...
export const findManyPosts = async (): Promise<TPostWithRelations[]> => {
  return (await db.query.PostTable.findMany({
    with: {
      ...
      comments: {
        with: { comment: true },
        columns: { commentId: false, postId: false },
      },
...
export const findSuggestedPosts = async (
  postsId: string[],
...
export const findFirstPostWithPostId = async (
  postId: string,
): Promise<TPostWithRelations> => {
...
export const findFirstPostWithUserId = async (
  userId: string,
): Promise<TPostWithRelations> => {
  return (await db.query.PostTable.findFirst({
...
export const findFirstLike = async (
  currentUserId: string,
  postId: string,
): Promise<TInferSelectPostLike> => {
  return (await db.query.PostLikeTable.findFirst({
    where: (table, funcs) =>
      funcs.and(
        funcs.eq(table.postId, postId),
        funcs.eq(table.userId, currentUserId),
      ),
  })) as TInferSelectPostLike;
};

export const findManyPostByUserId = async (
  userId: string,
): Promise<TPostWithRelations[]> => {
  return (await db.query.PostTable.findMany({
    where: (table, funcs) => funcs.eq(table.userId, userId),
...
export const insertLikePost = async (
  userId: string,
  postId: string,
): Promise<void> => {
...
export const deleteLikePost = async (
  userId: string,
  postId: string,
): Promise<void> => {
  await db
    ...
    .where(
      and(eq(PostLikeTable.userId, userId), eq(PostLikeTable.postId, postId)),
    );
};

export const updatePost = async (
  postId: string,
  values: { text: string; image: string },
): Promise<TInferSelectPost> => {
...
export const getPostCreatorAndId = async (
  postId: string,
...
export const findManyLikes = async (
  currentUserId: string,
): Promise<TInferSelectPostLike[]> => {
...
export const checkPostAuthor = async (
  ...
  const desiredPost: { userId: string } | undefined =
