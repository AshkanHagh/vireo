...
export const insertSavePost = async (
  postId: string,
  currentUserId: string,
): Promise<void> => {
  const postCache: { id: string } = await getHashWithIndexCache(
    `post:${postId}`,
    "id",
  );

  if (!postCache || Object.keys(postCache).length == 0) {
...
export const findFirstSave = async (
  postId: string,
  currentUserId: string,
): Promise<TInferSelectSavePost> => {
...
export const deleteSave = async (
  ...
  await db
    .delete(SavePostTable)
    .where(
      and(
        eq(SavePostTable.postId, postId),
        eq(SavePostTable.userId, currentUserId),
      ),
    );
};

export const findManySavedPostsId = async (currentUserId: string) => {
  return await db.query.SavePostTable.findMany({
