...
export const insertComment = async (
  ...
  const newComment = await db.transaction(async (trx) => {
    ...
    await trx
      ...
      .values({ postId, commentId: newComment.id });
...
export const findFirstComment = async (
  commentId: string,
  currentUserId: string,
  trx = db,
): Promise<TSelectComment> => {
  const comment: TSelectComment | undefined =
    await trx.query.CommentTable.findFirst({
...
export const deleteFirstComment = async (
  commentId: string,
  currentUserId: string,
): Promise<void> => {
...
export const findManyCommentsByPostId = async (
  postId: string,
...
export const findManyComments = async (
  ...
  return await db.query.CommentTable.findMany({
    where: (table, funcs) => funcs.eq(table.authorId, currentUserId),
    columns: { id: true },
  });
};
...
export const insertReplay = async (
  ...
  const [newReplay]: TInferSelectReplies[] = await db
    .insert(RepliesTable)
    .values({
...
const checkReplayAuthor = async (
  ...
  const replayToModifyCache: TInferSelectReplies | undefined = JSON.parse(
    await getHashWithIndexCache(`comment_replies:${commentId}`, replayId),
  );
  replayToModify = replayToModifyCache;

  if (!replayToModifyCache || Object.keys(replayToModifyCache).length == 0) {
    replayToModify = await db.query.RepliesTable.findFirst({
      where: (table, funcs) => funcs.eq(table.id, replayId),
    });
  }
...
export const updateReplay = async (
  ...
  const [updatedReplay]: TInferSelectReplies[] = await db
    .update(RepliesTable)
    .set({ text: replayText })
...
export const findAllReplies = async (
  commentId: string,
  limit: number,
  offset: number,
) => {
