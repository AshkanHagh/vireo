...
postEventEmitter.on(
  "create-post",
  async (currentUserId: string, createdPostId: string): Promise<void> => {
...
postEventEmitter.on("post_cache", async (postId: string): Promise<void> => {
  ...
  const likesUserInfo = likes?.map((user) => user.user.id);

  const commentsInfo = comments?.map((comment) => comment.comment.id);
  ...
  const fixedResult = {
    ...
    likes: JSON.stringify(likesUserInfo),
    tags,
...
postEventEmitter.on("update-post-cache", async (): Promise<void> => {
  ...
  for (const post of posts) {
    ...
    const likesUserInfo = likes?.map((user) => user.user);
    const commentsInfo = comments?.map((comment) => comment.comment);
...
postEventEmitter.on(
  "delete-post",
  async (userId: string, postId: string): Promise<void> => {
    await Promise.all([
      deleteFirstPost(postId),
      deleteFromCache(`post:${postId}`),
...
postEventEmitter.on(
  ...
  async (
    ...
    const likesArray: string[] = await getListScore(
      `posts_liked:${currentUserId}`,
    );

    const suggestionPostScore: Record<string, string> = arrayToKeyValuePairs(
      await getListScore(`suggest_post:${creatorId}`),
    );
