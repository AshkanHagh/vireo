...
import {
  getFollowings_Of_followings,
  getTopFollowedUsers,
  getTopLikedPostsCreators,
...
userEventEmitter.on(
  ...
  (
    userId: string,
    userHashValue: TUserWithProfileInfo,
    profileValue: TInferSelectUserProfile,
  ) => {
...
userEventEmitter.on(
  "addSuggestedUsersToCache",
  (value: TUserWithRelations[], currentUserId: string) => {
    value.forEach(async (user) => {
      await addHashListCache(
        `top_followers:${currentUserId}`,
        user.id,
        user,
        604800,
      );
...
userEventEmitter.on(
  "updateSuggestedUsersCache",
  async (currentUserId: string) => {
    await Promise.all([
      getTopLikedPostsCreators(currentUserId),
      getFollowings_Of_followings(currentUserId),
...
userEventEmitter.on(
  ...
  async (
    ...
    await Promise.all([
      insertFollow(currentUser.id, userToFollowId),
      addHashListCache(
...
userEventEmitter.on(
  "unfollow",
  async (currentUser: TInferSelectUserNoPass, userToFollowId: string) => {
    await Promise.all([
      deleteFollow(currentUser.id, userToFollowId),
