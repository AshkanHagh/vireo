...
export const sendToken = (
  user: TUserWithProfileInfo,
  res: Response,
  tokenFor: "login" | "refresh",
) => {
  ...
  const userInfo = combineUserProfileWithUser(
    (filteredProfile["profile"] as TInferSelectUserProfile) || null,
...
export const decodedToken = (refreshToken: string) => {
  return jwt.verify(refreshToken, process.env.REFRESH_TOKEN) as JwtPayload &
    TInferSelectUser;
};
