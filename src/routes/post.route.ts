...
router.post(
  "/tags/:postId",
  [validationMiddleware(tagBody), isAuthenticated],
  addTagToPost,
);
...
router.post(
  ...
  [validationMiddleware(commentBody), isAuthenticated],
  addReplay,
);

router.patch(
  "/comment/replay/:commentId/:replayId",
  [validationMiddleware(commentBody), isAuthenticated],
  editReplay,
);
...
router.post(
  "/",
  [isAuthenticated, validationMiddleware(createPostBody)],
  createPost,
);
