...
export const registerService = async (
  ...
  try {
    const isUserExists: TInferSelectUserNoPass | undefined =
      await findFirstUser(email, username, undefined);
...
export const loginService = async (
  ...
  try {
    const isUserExists: TUserWithProfileInfo | undefined = await findFirstUser(
      email?.toLowerCase(),
      username?.toLowerCase(),
      undefined,
    );
...
export const refreshTokenService = async (
  ...
  try {
    ...
    const session: TUserWithProfileInfo = await getAllFromHashCache(
      `user:${decoded.id}`,
    );
