...
import type {
  TErrorHandler,
  TFixedCommentReplies,
  TFixedPostComment,
  TInferSelectReplies,
  TPostCommentWithAuthor,
  TPostWithRelations,
  TRepliesRelations,
  TSelectComment,
  TUserProfile,
...
export const addCommentService = async (
  ...
  try {
    ...
    const desiredPostCache: string = await getHashWithIndexCache(
      `post:${postId}`,
      "userId",
    );
    ...
    const newComment: TSelectComment = await insertComment(
      currentUserId,
      postId,
      text,
    );
    const suggestionCount = await calculateNumberOfSuggestions(
      currentUserId,
      postId,
      "other",
      0.1,
    ...
    await Promise.all([
      ...
      addToListWithScore(
        `suggest_post:${desiredPost.userId}`,
        suggestionCount,
        desiredPost.id,
      ),
    ]);
...
export const editCommentService = async (
  ...
  try {
    const updatedComment: TSelectComment = await updateComment(
      commentId,
      currentUserId,
      text,
    );
...
export const deleteCommentService = async (
  ...
  try {
    ...
    const postToModify: TPostWithRelations = await getAllFromHashCache(
      `post:${postId}`,
    );
    ...
    await Promise.all([
      addToListWithScore(
        `suggest_post:${postToModify.userId}`,
        -suggestionCount,
        postToModify.id,
      ),
...
export const postCommentsService = async (
  ...
  try {
    ...
    const postCommentsCache: Record<string, string> = await getAllFromHashCache(
      `post_comments:${postId}`,
    );
    const parsedComments: TSelectComment[] = Object.values(
      postCommentsCache,
    ).map((comment) => JSON.parse(comment));

    const commentWithAuthor: TFixedPostComment[] = await Promise.all(
      parsedComments.map(async (comment) => {
        const author: TUserProfile = await getAllFromHashCache(
    ...
    if (!postCommentsCache || Object.keys(postCommentsCache).length == 0) {
      ...
      const modifiedPostComments: TFixedPostComment[] = postComments.map(
        (comment) => {
          const { id, authorId, author, createdAt, text, updatedAt } =
            comment.comment;
          ...
          return {
            ...
            author: {
              ...
              profilePic: author.profile?.profilePic,
            },
          };
        },
      );
...
export const addReplayService = async (
  commentId: string,
  currentUserId: string,
  replayText: string,
): Promise<TInferSelectReplies> => {
  try {
    const newReplay: TInferSelectReplies = await insertReplay(
      currentUserId,
      commentId,
      replayText,
    );
...
export const editReplayService = async (
  currentUserId: string,
  commentId: string,
  replayId: string,
  replayText: string,
  ...
  try {
    const updatedReplay: TInferSelectReplies = await updateReplay(
      currentUserId,
      commentId,
      replayId,
      replayText,
    ...
    addHashListCache(
      `comment_replies:${updatedReplay.commentId}`,
      replayId,
      updatedReplay,
      2419200,
    );
...
export const deleteReplayService = async (
  currentUserId: string,
  commentId: string,
  replayId: string,
): Promise<string> => {
...
export const commentRepliesService = async (
  commentId: string,
  startIndex: number,
  limit: number,
): Promise<TFixedCommentReplies[]> => {
  try {
    let commentReplies: TRepliesRelations[] | TFixedCommentReplies[];

    const commentRepliesCache: Record<string, string> =
      await getAllFromHashCache(`comment_replies:${commentId}`);
    const parsedReplies: TInferSelectReplies[] = Object.values(
      commentRepliesCache,
    ).map((replay) => JSON.parse(replay));

    const repliesWithAuthor: TFixedCommentReplies[] = await Promise.all(
      parsedReplies.map(async (replay) => {
        const author: TUserProfile = await getAllFromHashCache(
          `user:${replay.authorId}`,
        );
        return {
          ...replay,
          author: { username: author.username, profilePic: author.profilePic },
    ...
    if (!commentRepliesCache || Object.keys(commentRepliesCache).length == 0) {
      ...
      const modifiedCommentReplies: TFixedCommentReplies[] = commentReplies.map(
        (replay) => {
          const { id, commentId, authorId, createdAt, updatedAt, author } =
            replay;
