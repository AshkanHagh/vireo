...
): Promise<(TNotificationResult | TInferSelectNotification)[]> => {
  try {
    let notifications: (TNotificationResult | TInferSelectNotification)[];

    const cachedNotifications: string[] = await getAndUpdateListCache(
      `notification:${currentUserId}`,
    );
    if (cachedNotifications.length <= 0) {
      notifications = await findManyNotifications(currentUserId);
    }

    notifications = await Promise.all(
      cachedNotifications.map(async (notification) => {
        const notificationData: TInferSelectNotification =
          JSON.parse(notification);

        const userProfile: TUserProfile = await getAllFromHashCache(
          `user:${notificationData.from}`,
        );
...
export const combineNotificationToUser = (
  user: TUserProfile,
  notification: TInferSelectNotification,
): TNotificationResult => {
  const { to, type, read, createdAt, updatedAt } = notification;
  return {
    from: { username: user.username, profilePic: user.profilePic },
    to,
    type,
    read,
    createdAt,
