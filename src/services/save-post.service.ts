...
import {
  deleteSave,
  findFirstSave,
  findManySavedPostsId,
  insertSavePost,
} from "../database/queries/save-post.query";
import ErrorHandler from "../libs/utils/errorHandler";
import type {
  TErrorHandler,
  TInferSelectSavePost,
  TPostWithRelations,
...
export const savePostService = async (
  ...
  try {
    ...
    const currentUserSetListCache = await getFromSetListCache(
      `user:${currentUserId}:savedPosts`,
      postId,
    );
    if (!currentUserSetListCache) {
      alreadySaved = await findFirstSave(postId, currentUserId);
    }
...
export const savedPostsService = async (
  ...
  const savedPostsIdCache: string[] = await getAllSetListCache(
    `user:${currentUserId}:savedPosts`,
  );
  savedPostsId = savedPostsIdCache;

  if (!savedPostsIdCache) {
    const savedPostIdDB: { postId: string }[] =
      await findManySavedPostsId(currentUserId);
    savedPostsId = savedPostIdDB.map((post) => post.postId);
  }

  const savedPosts: TPostWithRelations[] = await Promise.all(
    savedPostsId.map(async (postId) => {
      let savedPosts: TPostWithRelations;

      const savedPostCache: TPostWithRelations = parseAndFixCacheResult(
        await getAllFromHashCache(`post:${postId}`),
      );
      savedPosts = savedPostCache;
      if (!savedPostCache || Object.keys(savedPostCache).length == 0) {
        savedPosts = await findFirstPostWithPostId(postId);
      }
